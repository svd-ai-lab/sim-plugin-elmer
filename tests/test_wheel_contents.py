...
hatchling picks it up via ``packages = ["src/sim_plugin_elmer"]``
...
def test_wheel_contains_skills(tmp_path: Path) -> None:
    ...
    wheels = list(out_dir.glob("sim_plugin_elmer-*.whl"))
    ...
    required = {
        "sim_plugin_elmer/__init__.py",
        "sim_plugin_elmer/driver.py",
        "sim_plugin_elmer/_skills/elmer/SKILL.md",
    }
